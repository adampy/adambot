...
class SupportConnection:
    def __init__(self, id, member_id, staff_id, started_at, guild_id):
        self.id = id
    ...
    @classmethod
    async def create(cls, bot: commands.Bot, id, member_id, staff_id, started_at, guild_id = 0):
        """Classmethod that allows async code to be executed, and therefore the staff and user objects
        to be obtained from the bot."""
        self = SupportConnection(id, member_id, staff_id, started_at, guild_id)
        self.bot = bot
        if member_id != 0:
...
class Support(commands.Cog):
    ...
    async def on_message(self, message):
        if message.guild is None and not message.author.bot:  # Valid DM message
            ...
            if not connection and starts_with_any(message.content.lower(), ['support start', '-support start', '-support begin', 'support begin']):
                ...
                try: # Try clause gets a valid guild_id
                    ...
                    if not guild_id in [g.id for g in self.bot.guilds]:
                        await message.author.send("That is not a guild I know of, to get a list of guilds type `-support start`")
                        return
                except ValueError:
                    await message.author.send("That is not a guild I know of, to get a list of guilds type `-support start`")
                    return
                    ...
                    output = "To start a ticket, you must run this command with a guild ID, e.g. `-support start 1234567890`. Guild IDs of servers that we share are:\n"
            ...
            if starts_with_any(message.content.lower(), ['support end', 'support close', 'support finish']) and connection:
                ...
                if not connection and not message.content.startswith('-'):
                    ...
                    await message.author.send('Hi! You are not in a support chat with a staff member. If you would like to start an anonymous chat with an anonymous staff member type `-support start` to get started.')
                else:
                    # Doesn't start with -
                    if connection:
    ...
    async def support(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            await ctx.send('```-help support``` for more commands. If you want to open a ticket type ```-support start```')
