...
class Role(commands.Cog):
    ...
    async def find_closest_role(self, ctx, role, verbosity=0):
        """
        Verbosity:
          0: Silence
          1: Send embeds
    ...
    async def info(self, ctx, *, role):
        ...
        """
        role = await self.find_closest_role(ctx, role, verbosity=2)
        if len(role) > 1:
            return
        role = role[0]
    ...
    async def members(self, ctx, *, role_name):
        ...
        possible = await self.find_closest_role(ctx, role_name, verbosity=2)
    ...
    async def manage_roles_check(self, ctx, action, role: discord.Role=None, verbosity=2, error_title="default"):  # verbose toggle e.g. in multi-role changes
        """
    ...
    async def checked_role_change(self, ctx, role: discord.Role, member: discord.Member, action: str, tracker: discord.Message=None, part_of_more=False, single_output=True):
        ...
        return_check = 0 if check == 0 else 2
        if check == 0:
            try:
                if action == "remove":
    ...
    async def swap(self, ctx, swap_from, *, swap_to):
        ...
        swap_from = await self.find_closest_role(ctx, swap_from, verbosity=2)
        if len(swap_from) > 1:
            return
        swap_from = swap_from[0]

        swap_to = await self.find_closest_role(ctx, swap_to, verbosity=2)
        if len(swap_to) > 1:
            return
        swap_to = swap_to[0]
...
.
    ...
    async def addall(self, ctx, ref_role, *, add_role):
        ...
        ref_role = await self.find_closest_role(ctx, ref_role, verbosity=2)
        if len(ref_role) > 1:
            return
        ref_role = ref_role[0]

        add_role = await self.find_closest_role(ctx, add_role, verbosity=2)
        if len(add_role) > 1:
            return
        add_role = add_role[0]
    ...
    async def clear(self, ctx, *, member: discord.Member):
        ...
        for i in range(len(roles)):
            try:
                if await self.manage_roles_check(ctx, "remove", role=roles[i], verbosity=0) == 0:
                    await member.remove_roles(roles[i], reason=f"Requested by {ctx.author}")
                else:
                    fail.append(role.name[i])
