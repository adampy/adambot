...
async def insert_migrate_column_if_not_exists(pool: asyncpg.pool.Pool, table_name: str, column_name: str, column_params: str = None, rename_from: str = None, create_if_not_exists: bool = True) -> None:
    try:
        async with pool.acquire() as connection:
            available_columns = await connection.fetch(f"SELECT column_name FROM information_schema.columns WHERE table_name='{table_name}'")
    ...
    async with pool.acquire() as connection:
        for table in table_collection:
            ...
            for field_name in field_names:  # idea of this is to add new columns added to cog configs automatically without needing to do it manually
                await insert_migrate_column_if_not_exists(pool, name, field_name, column_params=field_cond[field_names.index(field_name)], rename_from=renames.get(field_name, None))
            ...
            for rename in renames:  # this is technically undefined behaviour since ordinarily configs shouldn't be requesting renames for columns that aren't specified but we're making use of it for config ;)
                if rename not in field_names:  # ie already done above
                    await insert_migrate_column_if_not_exists(pool, name, rename, rename_from=renames.get(rename), create_if_not_exists=False)  # won't know types + handled by config anyway to save db space
